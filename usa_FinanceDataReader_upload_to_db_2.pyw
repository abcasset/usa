...
sql_top10_days = 'SELECT stk_cd, astk_cd, stk_nm FROM A_STK_USA  ;'
...
# for 
df_main = pd.DataFrame(columns=['Date', 'stk_cd', 'astk_cd', 'stk_nm', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'])

for i in df_stk_cd.index:
    try:
        ls_cd = df_stk_cd.loc[i, 'stk_cd']
        ls_acd = df_stk_cd.loc[i, 'astk_cd']
...
df_main2 = df_main[['Date', 'stk_cd', 'astk_cd', 'stk_nm', 'Adj Close']]
...
sql = "insert into A_STK_DAILY_USA (TRD_DT, STK_CD, ASTK_CD, STK_NM, ADJ_CLOSE) values(%s, %s, %s, %s,   %d);"
